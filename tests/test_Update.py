...
# [] TODO переименовать все методы согласно update

@pytest.mark.order(5)
# где 1.json - Статья со статусом открыт (открытый тип офиса нет) - > в закрыт
# [] TODO подумать что делать с графиками, они по сути все должны быть пустыми и поле с загруженностью тоже
def test_get_status_value_open(db_connect):
    ...
    article_name = db_req.get_article_name_by_id(article_id).split()
    assert article_name[-1] == "(Закрыт)"
    assert "Закрыт" == db_req.get_code_value_by_tag(tmp.status)[0]


@pytest.mark.order(6)
# где 2.json - Статья со статусом открыт (открытый тип офиса любое кроме нет) ->
# временно закрыт
# проверить приписку (Временно закрыт) и статус временно закрыт
...
def test_get_status_value_closed_office_type(db_connect):
    ...
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    article_name = db_req.get_article_name_by_id(article_id)
    assert "(Временно закрыт)" in article_name
    assert "Временно закрыт" == db_req.get_code_value_by_tag(tmp.status)[0]


@pytest.mark.order(7)
# где 3.json - Статья со статусом закрыт -> открыт
# проверить статус открыт
...
@pytest.mark.order(9)
# где 4.json - Статья со статусом скоро откроется ->открыт
...
# где 5.json - Статья со статусом временно закрыт -> закрыт
...
# [] TODO надо обновить тестовые данные
# требования уточнены ПАО сокращается, филиал от сентябрьского релиза
@pytest.mark.skip(reason="неактуальный json")
@pytest.mark.order(11)
# 6 статья с филиалом fullofficename
# Проверить наличие филиала и совпадение по имени
def test_get_fullofficename(db_connect):
    ...
    tmp = xml_article.deserialize()
    assert (
        "№ 2351 Банка ВТБ (ПАО) в г. Краснодаре"
        == db_req.get_code_value_by_tag(tmp.branch.branch_code)[0]
    )


@pytest.mark.order(12)
# где 7.json - Статья с бизнес группой (rooBg) РОО «сладкий тест» -> новая бизнес группа
# Задача
# Проверить бизнес группу
def test_get_bisness_group(db_connect):
    ...
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    assert "новая бизнес группа" == tmp.branch.branch_business_group


@pytest.mark.order(13)
# 8 где 8.json - статья с городом бади -> накама
# Задача
# проверить город
def test_get_city(db_connect):
    ...
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    assert "накама" == (db_req.get_code_value_by_tag(tmp.branch.city))[0]


@pytest.mark.order(14)
# где 9.json - статья с адресом "г. Астрахань, ул. Яблочковe, д. 51 лит. Desert" ->"г. Астрахань, ул. Яблочковe, д. 51 лит. GTFO"
...
# 26 json Отделение работает в субботу (ФЛ) Проверить работоспособность
@pytest.mark.order(29)
def test_ibs(db_connect):
    db_con = db_connect
    db_req = DbRepository(db_con)
    article_id = db_req.get_article_id_by_code_tp("000026")
    active_ver_xml = db_req.get_active_ver(article_id)
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    assert "Работает в субботу" in db_req.get_code_value_by_tag(tmp.branch_features)


# 27 json Работает в будни после 19 (ФЛ) Проверить работоспособность
@pytest.mark.order(30)
def test_ibs(db_connect):
    db_con = db_connect
    db_req = DbRepository(db_con)
    article_id = db_req.get_article_id_by_code_tp("000027")
    active_ver_xml = db_req.get_active_ver(article_id)
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    assert "Работает в будни после 19" in db_req.get_code_value_by_tag(
        tmp.branch_features
    )


# 28.json - Подходит для маломобильных клиентов {hasRamp:"Y"} Проверить работоспособность
@pytest.mark.order(31)
def test_ibs(db_connect):
    db_con = db_connect
    db_req = DbRepository(db_con)
    article_id = db_req.get_article_id_by_code_tp("000028")
    active_ver_xml = db_req.get_active_ver(article_id)
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    assert "Подходит для маломобильных клиентов" in db_req.get_code_value_by_tag(
        tmp.branch_features
    )


# 29.json - Работает в воскресенье
@pytest.mark.order(32)
def test_ibs(db_connect):
    db_con = db_connect
    db_req = DbRepository(db_con)
    article_id = db_req.get_article_id_by_code_tp("000029")
    active_ver_xml = db_req.get_active_ver(article_id)
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    assert "Работает в воскресенье" in db_req.get_code_value_by_tag(tmp.branch_features)


# 30.json - Доступ в офис ограничен
@pytest.mark.order(33)
def test_ibs(db_connect):
    db_con = db_connect
    db_req = DbRepository(db_con)
    article_id = db_req.get_article_id_by_code_tp("000030")
    active_ver_xml = db_req.get_active_ver(article_id)
    xml_article = XmlParser(active_ver_xml)
    tmp = xml_article.deserialize()
    assert "Доступ в офис ограничен" in db_req.get_code_value_by_tag(
        tmp.branch_features
    )
